...
export async function GET(request: Request) {
  ...
  try {
    return NextResponse.json({ mode: "supabase", items: await listHeroSlides(supabase) });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ mode: "supabase", item: await createHeroSlide(supabase, body) });
  } catch (error) {
